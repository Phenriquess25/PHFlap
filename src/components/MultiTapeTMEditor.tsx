...
export default function MultiTapeTMEditor() {
  ...
  // Função para salvar máquina de Turing multi-fita
  const handleSave = useCallback(() => {
    ...
    link.href = url
    link.download = `multi-tape-tm-${Date.now()}.json`
    link.click()
      ...
      <div style={{ position: 'relative', background: '#fafafa', borderRight: '1px solid #ddd', overflow: 'hidden' }}>
        ...
        >
          ...
          <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
            ...
            {transitions.filter(t => t.from !== t.to).map(({ from, to, reads, writes, moves, idx }) => {
