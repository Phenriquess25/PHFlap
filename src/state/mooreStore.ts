...
import { MooreMachine, MooreTransition, TransducerStep } from '@model/transducers'
...
interface MooreStore {
  ...
  setMachine: (m: MooreMachine) => void
  ...
  setOutput: (id: StateId, output: string) => void
  beginTransition: (from: StateId) => void
  addTransition: (t: MooreTransition) => void
...
export const useMooreStore = create<MooreStore>((set, get) => ({
  ...
  addState: (pos) => {
    ...
    set({
      machine: { ...machine, states: [...machine.states, newId], outputs: { ...machine.outputs, [newId]: '' } },
  ...
  removeState: (id) => {
    const { machine, positions } = get()
    const { [id]: _, ...newOutputs } = machine.outputs
    set({
      machine: {
        ...machine,
  ...
  startSimulation: (input: string) => {
    const { machine } = get()
    import('@core/algorithms/simulateMoore').then(mod => {
      const result = mod.simulateMoore(machine, input)
